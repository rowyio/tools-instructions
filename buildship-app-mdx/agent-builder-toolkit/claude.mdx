...
export default function Layout(props) {
  ...
  const apiKey = props.toolkit?.apiKeys[0];
  const projectId = props.projectId;
  const flowId = props.toolkit?.tools?.map(t => t.id).join(",");
  const flowLabel = props.flowLabel;
  const outputFolder = apiKey ? props.toolkit?.name : "";
  const toolkitId = props.toolkit?.id;
  const command = `npx -y buildship-tools create-mcp-server --project ${projectId} --flows ${flowId} --apiKey ${apiKey} --toolkitId ${toolkitId} --outputFolder ${camelize(outputFolder)} --client claude --token ${idToken}`;
